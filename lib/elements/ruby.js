...
(function () {
    ...
    ruby.newSegmentAndCategoriseRubyElement = function (el) {
        ...
        var processRubyChild = function () {
            ...
            // if we have current annotations and the node is IEWS don't commit
            // this leaves it in whatever current range but does not create spurious automatic bases
            if (currentAnnotations.length && helpers.isInterElementWhitespace(currentChild)) {
                index++;
                return processRubyChild();
            }
